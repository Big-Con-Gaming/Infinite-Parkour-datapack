...
/spawn
  ...
  execute at @e[type=marker,tag=ip_jump_next,distance=..512] as @n[type=block_display,distance=..0.01,tag=ip_block_display] run
  ...
  /place_jump_objects
    ...
    %EMPTY%
      ...
      $execute if data storage infinite_parkour:calc temp_current_block{type:"$(id)"} run
  ...
  execute positioned ~-31.5 ~-31.5 ~-0.5 as @e[type=marker,tag=ip_jump_next,dx=64,dy=64,dz=64] at @s run
    ...
    %EMPTY%
      ...
      %EMPTY%
        ...
        execute if data entity @s data.rotational run
