...
from python import c, t
def fetch(jumppack_id):
  c(f"data modify storage infinite_parkour:jumppack jumppack set from storage jumppack_{jumppack_id}:data jumppack")
def delete(jumppack_id):
  c(F"data remove storage jumppack_{jumppack_id}:data jumppack")
def get_jump(jumppack_id, page, row, col):
  c(f'data modify storage infinite_parkour:calc jump set from storage jumppack_{jumppack_id}:data jumppack.jumps[{page}][{row}][{col}]')
def update_list(jumppack_id):
  c(f'execute unless data storage infinite_parkour:jumppack list run data modify storage infinite_parkour:jumppack list set value []')
  c(f"execute if data storage jumppack_{jumppack_id}:data jumppack unless data storage infinite_parkour:jumppack list["+"{name:'"+jumppack_id+"}] run data modify storage infinite_parkour:jumppack list append value {name:'"+jumppack_id+"'}")
  c(f"execute unless data storage jumppack_{jumppack_id}:data jumppack run data remove storage infinite_parkour:jumppack list["+"{name:'"+jumppack_id+"'}]")
def set_jump(jumppack_id, page, row, col):
  # make sure there is a spot for the jump
  c(f'execute unless data storage jumppack_{jumppack_id}'+':data jumppack run data modify storage jumppack_'+jumppack_id+':data jumppack set value {jumps:[]}')
  c(f'execute unless data storage jumppack_{jumppack_id}data jumppack.jumps[{page}] run data modify storage jumppack_{jumppack_id}:data jumppack.jumps set value [[],[],[],[],[],[],[],[]]')
  c(f'execute unless data storage jumppack_{jumppack_id}:data jumppack.jumps[{page}][{row}] run data modify storage jumppack_{jumppack_id}:data jumppack.jumps[{page}] set value [[],[],[],[],[],[]]')
  c(f'execute unless data storage jumppack_{jumppack_id}:data jumppack.jumps[{page}][{row}][{col}] run data modify storage jumppack_{jumppack_id}:data jumppack.jumps[{page}][{row}] set value [{{}},{{}},{{}},{{}},{{}}]')
  # place jump
  c(f'data modify storage jumppack_{jumppack_id}:data jumppack.jumps[{page}][{row}][{col}] set from storage infinite_parkour:calc jump')
  # clean up if possible
  c(f'execute unless data storage jumppack_{jumppack_id}:data jumppack.jumps[{page}][{row}][].blocks run data modify storage jumppack_{jumppack_id}:data jumppack.jumps[{page}][{row}] set value []')
  c(f'execute unless data storage jumppack_{jumppack_id}:data jumppack.jumps[{page}][][].blocks run data modify storage jumppack_{jumppack_id}:data jumppack.jumps[{page}] set value []')
  c(f'execute unless data storage jumppack_{jumppack_id}:data jumppack.jumps[][][].blocks run data remove storage jumppack_{jumppack_id}:data jumppack')
  # updating list
