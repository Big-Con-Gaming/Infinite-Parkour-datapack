...
const folder = mcf.lane;
export const {alloc, free, freeAll, tick, teleport_entry, exit} = folder;
const {claim, anonymous: a} = folder;

alloc`
  execute unless data storage infinite_parkour:lane free_positions[0] run ${a`
    scoreboard players add lobby_count ip_data 1
    scoreboard players operation #new ip_lane = lobby_count ip_data
	`}
  execute if data storage infinite_parkour:lane free_positions[0] run ${a`
  ...
  ${claim} with storage infinite_parkour:macro data
...
claim`
	$execute in infinite_parkour:lane positioned $(lobby_x) 0 0 run ${a`
		tp @s ~ ~ ~ 0 0
...
freeAll`
  execute in infinite_parkour:lane as @e[type=marker,tag=ip_lane_entry,distance=0..] at @s run ${free}
...
tick`
  ...
  execute as @a at @s run ${a`
    execute unless score @s ip_lane matches 1.. run ${a`
      # make sure the player is not in a lane
      execute if dimension infinite_parkour:lane run ${exit}
		`}
    execute if score @s ip_lane matches 1.. run ${a`
      # make sure the player is in the correct lane
      execute unless dimension infinite_parkour:lane run ${exit}
      ...
      execute unless score @s ip_lane = #test ip_lane run ${exit}
      ...
      execute positioned ~-512 -0.5 -0.5 run tag @n[type=marker,tag=ip_lane_entry,dx=1024,dy=1,dz=1] remove ip_lane_remove
		`}
	`}
  execute in infinite_parkour:lane as @e[type=marker,tag=ip_lane_remove,distance=0..] at @s run ${free}
  
  execute in infinite_parkour:lane as @e[type=interaction,tag=ip_leave,distance=0..] run ${a`
    execute on attacker run ${exit}
    execute on target run ${exit}
...
teleport_entry`
  ...
  execute in infinite_parkour:lane run ${a`$tp @s $(lobby_x) 0 0 0 0`} with storage infinite_parkour:macro data
  data remove storage infinite_parkour:macro data
`;
